...
class DecisionModel_2(object): #decision demo1:只针对没有9的情况(只有78)
    def __init__(self, machine_num_of_type):
        ...
        self.factor_456_raw_num = 1 
        self.factor_456_num = 0  
        self.factor_7_need_456num = 0.01
    ...
    def decision(self, robot_state_list
        ...
        if interupt_4567: # 如果不为空，interupt_4567是个实例化列表[classx,classy,....]
            ...
            for product_finish_class in interupt_4567:  # 列表里是一个个machine类
                    ...
                    if (product_finish_class.type == 7) : # 如果是7,并且没被锁住
            ...
            for index_2, machine in enumerate(machine_type_classlist):   # machine_type_classlist:[class4_1,class4_2,..]
                ...
                if machine.receive(robot_i.take_obj) & (not (robot_i.take_obj in machine.lock_list)) : # 目标这个原料格没满，并且这个格子没被别人锁住
                    ...
                    if machine.type == 4: # 如果是工作台4原料格没满，直接锁了
                        ...
                        D_mid = ((machine.x-robot_i.x)**2 + (machine.y-robot_i.y)**2) * (1/(self.type7_need_num[machine.type]+ self.factor_7_need_456num))  * (1/(machine.raw_num + self.factor_456_raw_num)) # *  (1/(self.machine_num_of_type[machine.type] + self.factor_456_num)) # 按照公式去找4,5,6 D = d * type7_need_type4_num * type_receive_is_empty
    ...
    def buyInterupt(self, machine, robot_state_list, machine_index_to_type_list, frame_id): # 找到小车去买4,5,6，并且不管他之前的目标是谁，都重新把目标设为4,5,6，如果没携带物品的话   *********** 但是有问题，如果场上现在不需要4,5,6，不能硬买
        ...
        # 逻辑修改: 就算有小车选了这个为目标，也可以实时更改，让更近的小车来选择它为目标
        # 遍历4个小车，找到有空去买这个产品的小车
        for robot_i in robot_state_list: 

            # 如果不是0，即已经携带了物品，就不选这个小车  *********************这个地方修改的话可以作为demo4
        ...
        if min_D_robot != None:    # 确实有小车可以去办这事，首先是设定target，然后锁一下************************
            ...
            self.clearLastTarget(min_D_robot, robot_state_list, machine) # 消除掉上一个以它为目标的小车的target
            # machine.buyerLock()  # 上买锁 *********************************demo2主要优化：不上买锁
            if machine.type != 7:
                self.type7_need_num[machine.type] -= 1 

   # 把上一个以它为目标的小车的target置为[]
    def clearLastTarget(self, min_D_robot_now, roobot_state_list, machine):
        for robot_i in roobot_state_list:
            if robot_i.target != []:
                if robot_i.target == machine.id:
                    robot_i.target = []
    ...
    def calculateTimeEnough(self, robot_i, machine, frame_id):
        ...
        if machine.type != 7: #先暂时不考虑4,5,6了，可能出bug
            ...
            if 9000 - frame_id <= 800: #如果剩余时间太短就不去拿了，不知道如何计算剩余时间，就先设个常数吧。
                return True
            else:
                return False
