...
class DecisionModel_2(object): #decision demo1:只针对没有9的情况(只有78)
    ...
    def pickSellerMachine(self, machine_state_list, machine_index_to_type_list, robot_i): 
        ...
        for index_1,machine_type_classlist in enumerate(machine_state_list[0:3]): # machine_dict:[[class1_1..],[class2_1,..],[class3_1]]
            ...
            # 如果目标没被别人选了, 其实1,2,3就算被人锁了也可以去买，生产太快了，但是可能撞,所以先上个锁判断
                ...
                D_mid = (5000 - (machine.x-robot_i.x)**2 - (machine.y-robot_i.y)**2) * self.type123_need_num[machine.type]  #* 1/self.machine_num_of_type[machine.type]  # factor要考虑现在场上还需要多少1,2,3；优先去拿需要的多的，先把4,5,6填满再说
                # sys.stderr.write(str(self.type123_need_num)+'\n')
                if( (D_mid > D_max)): 
    ...
    def pickBuyerMachine(self, machine_dict, robot_i, machine_index_to_type_list): 
        ...
        if robot_i.take_obj in (1,2,3):  
            if robot_i.take_obj == 1:
                self.selectMinDMachineId_456(robot_i, machine_dict[3:5]) # 找买家
                if self.min_D_machine_id != -1:
                    self.type123_need_num[1] -= 1 # 场上共需的物料1的数量-1
            
            elif robot_i.take_obj == 2:
                self.selectMinDMachineId_456(robot_i, [machine_dict[3],machine_dict[5]]) # 找买家
                if self.min_D_machine_id != -1:
                    self.type123_need_num[2] -= 1 # 场上共需的物料2的数量-1
            else:
                self.selectMinDMachineId_456(robot_i, machine_dict[4:6]) # 找买家
                if self.min_D_machine_id != -1:
    ...
    # 根据最小D选4,5,6买家
    def selectMinDMachineId_456(self, robot_i, machine_dict):
        ...
        for index_1,machine_type_classlist in enumerate(machine_dict): # machine_dict:[[class4_1..],[class5_1,..],[class6_1]]
            ...
            for index_2, machine in enumerate(machine_type_classlist):   # machine_type_classlist:[class4_1,class4_2,..]
                ...
                if (robot_i.id == 1) | (robot_i.id == 0) | (robot_i.id == 2): 
                    sys.stderr.write(str(robot_i.id)+' with '+str(robot_i.take_obj)+' find buyer 456, machine and lock_lis:' + str(machine.id)+' '+str(machine.lock_list)+'\n')
                    sys.stderr.write(' machine can reveive? '+str(machine.receive(robot_i.take_obj))+' be locked? ' + str(not (robot_i.take_obj in machine.lock_list))+'\n')
                ...
                if machine.receive(robot_i.take_obj) & (not (robot_i.take_obj in machine.lock_list)) : # 目标这个原料格没满，并且这个格子没被别人锁住
                    if (robot_i.id == 1) | (robot_i.id == 0) | (robot_i.id == 2): sys.stderr.write('robot in here\n')
                    if machine.type == 4: # 如果是工作台4原料格没满，直接锁了
                        min_D_machine_id = machine.id
                        break_flag = True
                        break

                    else:
                        D_mid = (5000 - ((machine.x-robot_i.x)**2 + (machine.y-robot_i.y)**2)) * (self.type7_need_num[machine.type]+self.factor_pick_456) # * (1/(machine.raw_num + self.factor_456_raw_num)) # *  (1/(self.machine_num_of_type[machine.type] + self.factor_456_num)) # 按照公式去找4,5,6 D = d * type7_need_type4_num * type_receive_is_empty
                        if(D_mid > D_max): # 判断加权，权重是不是更大，更大的优先级越高
            ...
            if break_flag:
                break
        if (robot_i.id == 1) | (robot_i.id == 0) | (robot_i.id == 2): sys.stderr.write('robot '+str(robot_i.id)+' have '+str(robot_i.take_obj) +' to '+ str(min_D_machine_id)+'\n')
        self.min_D_machine_id = min_D_machine_id
