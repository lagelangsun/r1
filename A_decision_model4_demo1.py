...
class DecisionModel_4(object): #decision demo1:只针对没有9的情况(只有78)
            ...
            for index_1,machine in enumerate(machine_dict[6]): # machine_dict:[[class1_1..],[class2_1,..],[class3_1]]
                ...
                if robot_i.take_obj not in machine.lock_list: 
                    ...
                    if (D_mid < D_min): 
                        min_D_machine_id = machine.id
                        D_min = copy(D_mid)
            if robot_i.id == 2: sys.stderr.write(str(robot_i.id)+'in 4,5,6 last to ' + str(min_D_machine_id)+'\n')
        ...
        for index_1,machine_type_classlist in enumerate(machine_dict): # machine_dict:[[class4_1..],[class5_1,..],[class6_1]]
            ...
            for index_2, machine in enumerate(machine_type_classlist):   # machine_type_classlist:[class4_1,class4_2,..]
                ...
                if machine.receive(robot_i.take_obj) & (not (robot_i.take_obj in machine.lock_list)) : # 目标这个原料格没满，并且这个格子没被别人锁住
                    ...
                    if (machine.type == 4) & (self.type7_need_num[4] != 0): # 如果是工作台4原料格没满，直接锁了
    ...
    def buyInterupt(self, machine, robot_state_list, machine_index_to_type_list, frame_id): # 找到小车去买4,5,6，并且不管他之前的目标是谁，都重新把目标设为4,5,6，如果没携带物品的话   *********** 但是有问题，如果场上现在不需要4,5,6，不能硬买
        ...
        for robot_i in robot_state_list: 
            ...
            if robot_i.target != []: # 如果已经有小车以这个为目标去卖了，就别让别的再把它设为目标了吧
                ...
                if (robot_i.target == [machine.id,0,1]) & (math.sqrt((machine.x-robot_i.x)**2 + (machine.y-robot_i.y)**2) >1000):
