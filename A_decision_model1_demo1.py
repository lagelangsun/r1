...
class DecisionModel_1(object): #decision demo1:只针对没有9的情况(只有78)
    ...
    def pickSellerMachine(self, machine_state_list, robot_i): 
        ...
        if robot_i.id in (0,1):
            self.pickSellerD(machine_state_list[0], robot_i)
        elif robot_i.id == 2:
            self.pickSellerD(machine_state_list[1], robot_i)
        else:
            self.pickSellerD(machine_state_list[2], robot_i)
    ...
    def pickSellerD(self, machine_type_classlist, robot_i):
        min_D_machine_id = -1
        D_max = 0 
    ...
    def pickBuyerMachine(self, machine_dict, robot_i, machine_index_to_type_list): 
        ...
        if robot_i.take_obj in (1,2,3):  
            if robot_i.take_obj == 1:
                self.selectMinDMachineId_456(robot_i, machine_dict[3:5], machine_index_to_type_list) # 找买家
                self.type123_need_num[1] -= 1 # 场上共需的物料1的数量-1
            
            elif robot_i.take_obj == 2:
                self.selectMinDMachineId_456(robot_i, [machine_dict[3],machine_dict[5]], machine_index_to_type_list) # 找买家
                self.type123_need_num[2] -= 1 # 场上共需的物料2的数量-1
            else:
                self.selectMinDMachineId_456(robot_i, machine_dict[4:6], machine_index_to_type_list) # 找买家
